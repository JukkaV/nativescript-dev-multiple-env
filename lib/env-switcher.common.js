...
class EnvSwitcherCommon {
    ...
    isFileEqual(sourcePath, destinationPath) {
        if (fs.existsSync(sourcePath) && fs.existsSync(destinationPath)) {
            ...
            const destinationLines = destinationFile.toString().split(/(?:\r\n|\r|\n)/g);
            if (destinationLines.length > 0) {
                const firstLineParts = destinationLines[0].split()
            }
    ...
    getCurrentPackageId() {
        let nativescriptConfigFile = path.join(this.projectData.projectDir, 'nativescript.config.ts');
        ...
        let currentNativescriptConfigFile = fs.readFileSync(nativescriptConfigFile).toString();
        const oldIdSearch = /id: '([A-Za-z]{1}[A-Za-z\d_]*\.)*[A-Za-z][A-Za-z\d_]*'/;
        const match = currentNativescriptConfigFile.match(oldIdSearch);
